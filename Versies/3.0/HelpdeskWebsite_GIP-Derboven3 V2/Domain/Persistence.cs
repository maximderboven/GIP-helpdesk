...
namespace Domain
{
    class Persistence
    {
        ...
        {
            string sql = "UPDATE dbhelpdesk.tbltickets set dtmOpgelost=@nu WHERE id=@ticketID;";
            ConnectieMaken(sql);
            _cmd.Parameters.AddWithValue("@ticketID", ticketID);
            _cmd.Parameters.AddWithValue("@nu", DateTime.Now);
            _conn.Open();
            _cmd.ExecuteReader();
        }
        ...
        {
            string sql = "UPDATE dbhelpdesk.tblantwoorden set statusID=@statusid WHERE ticketID=@ticketID;";
            ConnectieMaken(sql);
            _cmd.Parameters.AddWithValue("@ticketID", ticketID);
            _cmd.Parameters.AddWithValue("@statusid", 3);
            _conn.Open();
            _cmd.ExecuteReader();
        }*/
        ...
        public int count_gebruikers_soort(string soort)
        {
            ...
            _cmd.Parameters.AddWithValue("pgebruiker", soort);
            _conn.Open();
        ...
        public Gebruiker read_gebruiker_id( int gebruikerID)
        {
            ...
            _conn.Open();
            _cmd.Parameters.AddWithValue("pgID", gebruikerID);
            MySqlDataReader dr = _cmd.ExecuteReader();
            while (dr.Read())
            {
                gebruiker = new Gebruiker(Convert.ToInt32(dr["id"]), dr["naam"].ToString(), dr["email"].ToString(), dr["groep"].ToString(), Convert.ToBoolean(dr["actief"]), Convert.ToDateTime(dr["dtmGeregistreerd"]));
